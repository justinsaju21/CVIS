...
import { fetchRecent, fetchRecommendation, sendChat, fetchVehicles } from '@/lib/api'
import type { TelemetryRow, WsEvent, FleetVehicle } from '@/lib/types'
...
const KEY_SUGGESTIONS: Record<string, { icon: string; title: string; desc: string; priority: 'low' | 'med' | 'high' }[]> = {
  ...
  'Eco': [
    { icon: '\ud83c\udf3f', title: 'Eco Mode Active', desc: 'Regeneration maximized \u2014 sustain speed', priority: 'low' },
  ...
  'Sport': [
    { icon: '\u26a1', title: 'Performance Mode', desc: 'Motor drawing peak power \u2014 monitor thermal', priority: 'med' },
  ...
  'Battery Overheating': [
    { icon: '\ud83d\udd34', title: 'Thermal Alert', desc: 'Battery temp critical \u2014 reduce power', priority: 'high' },
  ...
  'Charging': [
    { icon: '\u26a1', title: 'Charging Active', desc: 'Session at optimal rate \u2014 est. 42 min to full', priority: 'low' },
...
function LockedFeature({ title, desc }: { title: string; desc: string }) {
  return (
    <div style={{
      ...
      gap: 10, height: '100%', minHeight: 80, padding: '16px 12px',
      background: 'linear-gradient(135deg,rgba(0,212,255,0.03) 0%,rgba(80,40,120,0.06) 100%)',
      border: '1px dashed rgba(0,212,255,0.15)', borderRadius: 4, textAlign: 'center',
    }}>
      ...
      <div style={{ padding: '4px 14px', borderRadius: 20, background: 'linear-gradient(90deg,rgba(0,212,255,0.15),rgba(120,60,200,0.15))', border: '1px solid rgba(0,212,255,0.2)', fontSize: 9, fontFamily: 'Space Mono', color: 'var(--cyan)', letterSpacing: '0.1em' }}>
        UPGRADE TO PRO
      </div>
    </div>
  )
...
function ModeSegments({ driveMode }: { driveMode: string }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 3 }}>
      {segs.map((s, i) => (
        <div key={s.key} style={{ width: 26, height: 5, borderRadius: 2, background: i <= active ? s.color : 'rgba(255,255,255,0.08)', boxShadow: i === active ? `0 0 8px ${s.color}88` : 'none', transition: 'all 0.4s' }} />
      ))}
    </div>
  )
}
...
function SpeedometerDial({ speed, accent }: { speed: number; accent: string }) {
  ...
  const arc = (r2: number, sA: number, eA: number) => {
    const s = { x: cx + r2 * Math.cos(toRad(sA)), y: cy + r2 * Math.sin(toRad(sA)) }
    const e = { x: cx + r2 * Math.cos(toRad(eA)), y: cy + r2 * Math.sin(toRad(eA)) }
    return `M ${s.x.toFixed(2)} ${s.y.toFixed(2)} A ${r2} ${r2} 0 ${(eA - sA) > 180 ? 1 : 0} 1 ${e.x.toFixed(2)} ${e.y.toFixed(2)}`
  }
  ...
  const ticks = Array.from({ length: 11 }, (_, i) => {
    ...
    return {
      x1: cx + (r - 7) * Math.cos(toRad(a)), y1: cy + (r - 7) * Math.sin(toRad(a)),
      x2: cx + r * Math.cos(toRad(a)), y2: cy + r * Math.sin(toRad(a)),
      tx: cx + (r - 17) * Math.cos(toRad(a)), ty: cy + (r - 17) * Math.sin(toRad(a)),
      val: Math.round(i * max / 10), major: i % 5 === 0,
    }
  })
  return (
    <svg width={180} height={155} viewBox="0 0 180 155">
      ...
      <motion.path d={arc(r, startA, startA + totalA)} fill="none" stroke={accent} strokeWidth={8} strokeLinecap="round"
        style={{ filter: `drop-shadow(0 0 8px ${accent}88)` }} strokeDasharray="1000"
        animate={{ strokeDashoffset: 1000 - pct * 445 }} initial={{ strokeDashoffset: 1000 }} transition={{ duration: 0.5, ease: 'easeOut' }} />
      {ticks.map((t, i) => (
        <g key={i}>
          <line x1={t.x1} y1={t.y1} x2={t.x2} y2={t.y2} stroke={t.major ? `${accent}77` : `${accent}33`} strokeWidth={t.major ? 2 : 1} />
          {t.major && <text x={t.tx} y={t.ty + 3} textAnchor="middle" fontSize={6.5} fill="rgba(255,255,255,0.3)" fontFamily="Space Mono">{t.val}</text>}
        </g>
      ))}
      <motion.line x1={cx} y1={cy} x2={nx} y2={ny} stroke={accent} strokeWidth={2.5} strokeLinecap="round"
        style={{ filter: `drop-shadow(0 0 5px ${accent})` }} animate={{ x2: nx, y2: ny }} transition={{ duration: 0.5 }} />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        ...
        <motion.circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth={7} strokeLinecap="round"
          strokeDasharray={circ} animate={{ strokeDashoffset: offset }} initial={{ strokeDashoffset: circ }}
          transition={{ duration: 0.8, ease: 'easeOut' }} style={{ filter: `drop-shadow(0 0 6px ${color}88)` }} />
        <text x={size / 2} y={size / 2} textAnchor="middle" dominantBaseline="central" fontSize={12} fontWeight={700} fill="white" fontFamily="Space Mono"
          style={{ transform: `rotate(90deg)`, transformOrigin: `${size / 2}px ${size / 2}px` }}>{Math.round(pct)}%</text>
      </svg>
...
export default function DriverPage() {
  ...
  useEffect(() => {
    ...
    fetchRecent(60, selectedVehicleId).then((rows: unknown) => {
      ...
      setHistory(data.map((r, i) => ({
        ...
        batt: r.battery_pct, speed: r.speed_kmh, temp: r.motor_temp_c,
      })))
  ...
  const handleWs = useCallback((ev: WsEvent) => {
    ...
    if (ev.event === 'telemetry' || ev.event === 'telemetry_backfill') {
      ...
      rows.forEach((r) => {
        if (r.device_id !== selectedVehicleId) return
        setLatest(r)
        ...
        if (['Motor Fault', 'Battery Overheating', 'Low Battery'].includes(r.mode)) {
          ...
          const msgs: Record<string, string> = {
            'Motor Fault': `Motor fault \u2014 0x${r.fault_code.toString(16).toUpperCase()}`,
            'Battery Overheating': `Batt temp critical: ${r.battery_temp_c.toFixed(0)}\u00b0C`,
            'Low Battery': `Low battery: ${r.battery_pct.toFixed(0)}% \u2014 ${r.range_km.toFixed(0)}km range`,
          }
          const type = r.mode === 'Motor Fault' || r.mode === 'Battery Overheating' ? 'fault' : 'warn'
          setAlerts(prev => [{ id, msg: msgs[r.mode] ?? r.mode, type, time: new Date().toLocaleTimeString() }, ...prev.slice(0, 7)])
        }
      })
    }
    if (ev.event === 'ai_recommendation' && ev.device_id === selectedVehicleId) {
      typewriterEffect(ev.recommendation)
    }
    if (ev.event === 'vehicle_ai_status' && ev.device_id === selectedVehicleId) {
      setIsPro(ev.enabled)
    }
  }, [selectedVehicleId])
  ...
  const typewriterEffect = (text: string) => {
    ...
    const iv = setInterval(() => {
      setAiRec(text.slice(0, i + 1)); i++
      if (i >= text.length) { clearInterval(iv); setAiTyping(false) }
    }, 18)
  }
  ...
  const handleChat = async () => {
    ...
    catch (e: unknown) { const em = e instanceof Error ? e.message : 'AI unavailable'; setChatHistory(h => [...h, { role: 'ai', text: `Error: ${em}` }]) }
  ...
  const enginePct = mode === 'Charging' ? 0 : mode === 'Motor Fault' ? 0 : mode === 'Eco' ? 55 : mode === 'Sport' ? 92 : 78
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#020406', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: 24, maxWidth: 800, width: '100%', padding: '0 20px' }}>
          <div style={{ textAlign: 'center', marginBottom: 20 }}>
            <h1 className="font-display" style={{ fontSize: 32, letterSpacing: '0.2em', color: 'var(--cyan)', textShadow: '0 0 30px rgba(0,212,255,0.3)' }}>CVIS FLEET REGISTRY</h1>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.4)', fontFamily: 'Space Mono', letterSpacing: '0.1em', marginTop: 8 }}>SELECT A VEHICLE TO INITIALIZE DRIVE CONSOLE</div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {vehicles.map(v => (
              <motion.button
                key={v.device_id}
                onClick={() => { setSelectedVehicleId(v.device_id); setIsPro(v.ai_enabled); }}
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, padding: '6px 12px', background: 'rgba(0,0,0,0.4)', borderRadius: 20 }}>
                  ...
                  <span style={{ fontSize: 9, fontFamily: 'Space Mono', color: v.active ? '#2ed573' : 'rgba(255,255,255,0.4)', letterSpacing: '0.1em' }}>{v.active ? 'ONLINE' : 'OFFLINE'}</span>
                </div>
              </motion.button>
            ))}
          </div>
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: 1580, margin: '0 auto', padding: '6px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <button
                onClick={() => { setSelectedVehicleId(null); setLatest(null); setHistory([]); setMaxSpeed(0); setTotalDist(0); setAlerts([]); }}
                style={{
                  width: 32, height: 32, borderRadius: '50%', background: 'rgba(0,212,255,0.05)',
                  border: '1px solid rgba(0,212,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  cursor: 'pointer', color: 'var(--cyan)', flexShrink: 0
                }}
              >
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: 'Orbitron', fontSize: 19, fontWeight: 700, color: 'white', letterSpacing: '0.1em' }}>
                    LUXURY DRIVE CONSOLE <span style={{ color: 'var(--cyan)' }}>// {selectedVehicleId}</span>
                  </span>
                </div>
                <div style={{ fontSize: 8.5, color: 'rgba(255,255,255,0.28)', fontFamily: 'Space Mono', letterSpacing: '0.1em', marginTop: 2 }}>
                  Advanced real-time performance &amp; vehicle interface
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
              ...
              {[
                ...
                <div key={pill.label} style={{ padding: '4px 11px', borderRadius: 20, background: `${pill.col}15`, border: `1px solid ${pill.col}44`, fontSize: 7.5, fontFamily: 'Space Mono', letterSpacing: '0.08em', color: pill.active ? pill.col : 'rgba(255,255,255,0.22)', transition: 'all 0.4s' }}>{pill.label}</div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
            ...
            <div style={{ ...P, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ ...V, fontSize: 32, color: latest && latest.battery_pct < 20 ? '#ff4757' : '#2ed573' }}>
                  {Math.round(latest?.battery_pct ?? 0)}<span style={{ fontSize: 13, color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>%</span>
                </div>
              </div>
            ...
            <div style={{ ...P, padding: '12px 14px' }}>
              ...
              <div style={{ ...V, fontSize: 28, color: 'white', marginTop: 4 }}>
                {mode === 'Charging' ? `${Math.round(((100 - (latest?.battery_pct ?? 0)) / 100) * 60)} min`
                  : latest && latest.speed_kmh > 5 ? `${Math.round((latest.range_km / latest.speed_kmh) * 60)} min`
                    : `${Math.round(latest?.range_km ?? 0)} km`}
              </div>
            ...
            <div style={{ ...P, padding: '12px 14px' }}>
              ...
              {isPro ? (
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 5, marginTop: 4 }}>
                  ...
                  <div style={{ display: 'flex', gap: 2, alignItems: 'flex-end', marginLeft: 4, marginBottom: 3 }}>
                    {Array.from({ length: 5 }, (_, i) => <div key={i} style={{ width: 4, height: 6 + i * 2, background: i < Math.round(ds.score / 2) ? '#ffb347' : 'rgba(255,255,255,0.08)', borderRadius: 1 }} />)}
                  </div>
                </div>
              ) : (
          ...
          {/* ROW 2: Left + Route Map + Right */}
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.13 }}
            style={{ display: 'grid', gridTemplateColumns: '236px 1fr 218px', gap: 10, alignItems: 'start' }}>
            {/* LEFT */}
              ...
              <div style={{ ...P, padding: '13px' }}>
                ...
                <svg width="100%" viewBox="0 0 180 78" style={{ overflow: 'visible' }}>
                  ...
                  {([{ cx: 38, cy: 20, psi: tires.fl, pos: 'FL' }, { cx: 142, cy: 20, psi: tires.fr, pos: 'FR' }, { cx: 38, cy: 58, psi: tires.rl, pos: 'RL' }, { cx: 142, cy: 58, psi: tires.rr, pos: 'RR' }] as const).map(w => {
                    ...
                    return (
                      <g key={w.pos}>
                        <rect x={w.cx - 14} y={w.cy - 11} width={28} height={22} rx={3} fill={`${col}16`} stroke={col} strokeWidth={0.8} style={{ filter: `drop-shadow(0 0 3px ${col}55)` }} />
                        <text x={w.cx} y={w.cy - 1} textAnchor="middle" fontSize={9} fontWeight={700} fill={col} fontFamily="Space Mono">{w.psi}</text>
                        <text x={w.cx} y={w.cy + 8} textAnchor="middle" fontSize={5.5} fill="rgba(255,255,255,0.28)" fontFamily="Space Mono">PSI</text>
                        <text x={w.cx} y={w.cy - 13} textAnchor="middle" fontSize={5} fill="rgba(255,255,255,0.22)" fontFamily="Space Mono">{w.pos}</text>
                      </g>
                    )
                  })}
                </svg>
                <div style={{ marginTop: 8 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 3 }}>
                    {['LOW', 'NORMAL', 'HIGH'].map(l => <span key={l} style={{ fontSize: 6, fontFamily: 'Space Mono', color: 'rgba(255,255,255,0.25)', letterSpacing: '0.08em' }}>{l}</span>)}
                  </div>
                </div>
              </div>
              {/* Key Suggestions */}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {keySugs.map((s, i) => (
                      <motion.div key={i} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.07 }}
                        style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 9px', background: s.priority === 'high' ? 'rgba(255,71,87,0.06)' : 'rgba(0,212,255,0.04)', border: `1px solid ${s.priority === 'high' ? 'rgba(255,71,87,0.18)' : 'rgba(0,212,255,0.09)'}`, borderRadius: 3 }}>
                        <div style={{ fontSize: 13, lineHeight: 1 }}>{s.icon}</div>
                        <div style={{ flex: 1 }}>
                          <div style={{ fontSize: 8.5, fontWeight: 700, color: s.priority === 'high' ? '#ff4757' : 'rgba(255,255,255,0.7)', letterSpacing: '0.04em' }}>{s.title}</div>
                          <div style={{ fontSize: 7.5, color: 'rgba(255,255,255,0.28)', lineHeight: 1.4, marginTop: 1 }}>{s.desc}</div>
                        </div>
                        <ChevronRight size={8} color="rgba(255,255,255,0.18)" />
                      </motion.div>
                    ))}
                  </div>
                ) : (
            ...
            <div style={{ ...P, minHeight: 290, display: 'flex', flexDirection: 'column' }}>
              <div style={{ padding: '13px 16px', borderBottom: '1px solid rgba(0,212,255,0.08)', display: 'flex', alignItems: 'center', gap: 8 }}>
                <Navigation size={9} color="var(--cyan)" />
                <div style={L9}>Route Map</div>
                <div style={{ ...SB, marginLeft: 4 }}>Live navigation &amp; turn-by-turn guidance</div>
              </div>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '20px 24px', background: 'repeating-linear-gradient(45deg,rgba(0,212,255,0.01) 0,rgba(0,212,255,0.01) 1px,transparent 1px,transparent 22px)' }}>
                <svg width="100%" height={170} viewBox="0 0 440 170" style={{ opacity: 0.16 }}>
                  ...
                  <circle cx={150} cy={95} r={7} fill="var(--cyan)" fillOpacity={0.85} />
                  <circle cx={150} cy={95} r={14} fill="none" stroke="var(--cyan)" strokeOpacity={0.3} strokeWidth={1} />
                  <circle cx={420} cy={58} r={6} fill="#ffb347" fillOpacity={0.9} />
                  <text x={380} y={48} fontSize={9} fill="#ffb347" fillOpacity={0.7} fontFamily="Space Mono">DEST</text>
                </svg>
                <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                  <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 18px', background: 'rgba(0,212,255,0.06)', border: '1px solid rgba(0,212,255,0.18)', borderRadius: 4 }}>
                    ...
                    <span style={{ fontSize: 9.5, fontFamily: 'Space Mono', color: 'var(--cyan)', letterSpacing: '0.12em' }}>ROUTE MAP &mdash; COMING SOON</span>
                  </div>
                  <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.2)', fontFamily: 'Inter', lineHeight: 1.6 }}>
                    Live navigation with turn-by-turn directions<br />will be integrated in the next release
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <div style={{ ...P, padding: '13px' }}>
                ...
                {alerts.length === 0 ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                    {[
                      { icon: '\u2713', label: 'All Systems Normal', col: '#2ed573', dim: 'rgba(46,213,115,0.08)' },
                      { icon: '!', label: tires.rl < 30 ? 'Tire Pressure Low \u2014 Rear Left' : 'Tire Pressure OK', col: tires.rl < 30 ? '#ffb347' : '#2ed573', dim: tires.rl < 30 ? 'rgba(255,179,71,0.08)' : 'rgba(46,213,115,0.06)' },
                      { icon: '\u2139', label: 'Upcoming Service in 2300 km', col: '#00d4ff', dim: 'rgba(0,212,255,0.06)' },
                    ].map((a, i) => (
                      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 8px', background: a.dim, borderRadius: 3, border: `1px solid ${a.col}28` }}>
                        <div style={{ width: 15, height: 15, borderRadius: '50%', background: `${a.col}22`, border: `1px solid ${a.col}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, color: a.col }}>{a.icon}</div>
                        <span style={{ fontSize: 8.5, color: 'rgba(255,255,255,0.6)', fontFamily: 'Inter', lineHeight: 1.3 }}>{a.label}</span>
                      </div>
                    ))}
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 5, maxHeight: 160, overflowY: 'auto' }}>
                    {alerts.slice(0, 5).map(a => (
                      <div key={a.id} style={{ padding: '6px 8px', background: a.type === 'fault' ? 'rgba(255,71,87,0.08)' : 'rgba(255,179,71,0.08)', border: `1px solid ${a.type === 'fault' ? 'rgba(255,71,87,0.22)' : 'rgba(255,179,71,0.22)'}`, borderRadius: 3 }}>
                        <div style={{ fontSize: 8.5, color: a.type === 'fault' ? '#ff4757' : '#ffb347', fontFamily: 'Space Mono', lineHeight: 1.3 }}>{a.msg}</div>
                        <div style={{ fontSize: 7.5, color: 'rgba(255,255,255,0.22)', marginTop: 2, fontFamily: 'Space Mono' }}>{a.time}</div>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={{ ...P, padding: '13px', flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    <BrainCircuit size={9} color="var(--cyan)" />
                    <span style={L9}>AI Advisory</span>
          ...
          {/* ROW 3: Speed + Efficiency + Power */}
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.19 }}
            style={{ display: 'grid', gridTemplateColumns: '236px 1fr 260px', gap: 10, alignItems: 'stretch' }}>
            {/* Speed Analytics */}
            <div style={{ ...P, padding: '13px' }}>
              <div style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Gauge size={8} color={mconf.accent} /><div style={L9}>Speed Analytics</div></div>
                <div style={SB}>Real-time Speed Monitor</div>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7 }}>
                {[
                  { label: 'AVG SPEED', val: `${Math.round(avgSpd)} km/h` },
                  { label: 'MAX SPEED', val: `${Math.round(maxSpeed)} km/h` },
                  { label: 'DISTANCE', val: `${totalDist.toFixed(1)} km`, span: 2 },
                ].map((s, i) => (
                  <div key={i} style={{ gridColumn: (s as { span?: number }).span ? `span ${(s as { span?: number }).span}` : undefined, padding: '6px 8px', background: 'rgba(255,255,255,0.025)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ ...L9, fontSize: 7 }}>{s.label}</div>
                    <div style={{ ...V, fontSize: 13, color: 'white', marginTop: 2 }}>{s.val}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ ...P, padding: '13px', display: 'flex', flexDirection: 'column' }}>
              <div style={{ marginBottom: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><TrendingUp size={8} color={isPro ? 'var(--cyan)' : 'rgba(255,255,255,0.18)'} /><div style={L9}>Efficiency Graph</div></div>
                <div style={SB}>Fuel Efficiency &amp; Consumption</div>
              </div>
              {isPro ? (
                <>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 8, flexWrap: 'wrap' }}>
                    {[{ col: '#00d4ff', label: 'Efficiency (km/l)' }, { col: '#ffb347', label: 'Consumption (L/100km)' }].map(l => (
                      <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <div style={{ width: 14, height: 2, background: l.col, borderRadius: 1 }} />
                        <span style={{ fontSize: 7.5, fontFamily: 'Space Mono', color: 'rgba(255,255,255,0.32)', letterSpacing: '0.07em' }}>{l.label}</span>
                      </div>
                    ))}
                    <div style={{ marginLeft: 'auto', fontSize: 9, fontFamily: 'Space Mono', color: mconf.accent }}>
                      AVG {(14 + (latest?.battery_pct ?? 80) / 20).toFixed(1)} km/l
                    </div>
                  </div>
            ...
            <div style={{ ...P, padding: '13px' }}>
              <div style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}><Zap size={8} color={mconf.accent} /><div style={L9}>Power Flow</div></div>
                <div style={SB}>Real-time Power Distribution &amp; Performance</div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                    <span style={{ ...L9, fontSize: 7.5 }}>Power Distribution</span>
                    <span style={{ fontSize: 8, fontFamily: 'Space Mono', color: '#00d4ff' }}>{enginePct}%</span>
                  </div>
                  <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)' }}>
                    <motion.div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg,#00d4ff,#2ed573)', boxShadow: '0 0 8px #00d4ff44' }} animate={{ width: `${enginePct}%` }} transition={{ duration: 0.6 }} />
                  </div>
                </div>
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3 }}>
                    <span style={{ ...L9, fontSize: 7.5 }}>Regeneration</span>
                    <span style={{ fontSize: 8, fontFamily: 'Space Mono', color: '#2ed573' }}>{regenPct}%</span>
                  </div>
                  <div style={{ height: 4, borderRadius: 2, background: 'rgba(255,255,255,0.06)' }}>
                    <motion.div style={{ height: '100%', borderRadius: 2, background: 'linear-gradient(90deg,#2ed573,#00d4ff88)', boxShadow: '0 0 8px #2ed57344' }} animate={{ width: `${regenPct}%` }} transition={{ duration: 0.6 }} />
                  </div>
                </div>
                <div style={{ padding: '8px 10px', marginTop: 2, background: 'rgba(255,255,255,0.025)', borderRadius: 3, border: '1px solid rgba(255,255,255,0.05)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div>
                    <div style={{ ...L9, fontSize: 7 }}>TOTAL OUTPUT</div>
                    <div style={{ ...V, fontSize: 16, color: mconf.accent, marginTop: 2 }}>{Math.round(totalKw)} <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>kW</span></div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ ...L9, fontSize: 7 }}>HORSEPOWER</div>
                    <div style={{ ...V, fontSize: 16, color: 'rgba(255,255,255,0.65)', marginTop: 2 }}>{totalHp} <span style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', fontWeight: 400 }}>HP</span></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <AnimatePresence>
        {chatOpen && (
          <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', stiffness: 280, damping: 28 }}
            style={{ position: 'fixed', bottom: 0, right: 24, width: 380, zIndex: 200, background: 'rgba(4,8,12,0.98)', border: '1px solid rgba(0,212,255,0.2)', borderRadius: '8px 8px 0 0', overflow: 'hidden', boxShadow: '0 -8px 40px rgba(0,212,255,0.08)' }}>
            ...
            <div style={{ height: 280, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {chatHistory.map((m, i) => (
                <div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '88%', padding: '8px 12px', borderRadius: m.role === 'user' ? '8px 8px 2px 8px' : '8px 8px 8px 2px', background: m.role === 'user' ? 'rgba(0,212,255,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${m.role === 'user' ? 'rgba(0,212,255,0.25)' : 'rgba(0,212,255,0.07)'}`, fontSize: 12, color: 'var(--text-primary)', lineHeight: 1.6 }}>{m.text}</div>
              ))}
            ...
            <div style={{ padding: '10px 16px', borderTop: '1px solid rgba(0,212,255,0.08)', display: 'flex', gap: 8 }}>
              <input value={chatMsg} onChange={e => setChatMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleChat()} placeholder="Ask about your vehicle..."
                style={{ flex: 1, background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.12)', borderRadius: 4, padding: '8px 12px', color: 'var(--text-primary)', fontSize: 12, outline: 'none', fontFamily: 'Inter' }} />
      ...
      {/* Chat FAB */}
      {!chatOpen && (
        <motion.button whileHover={{ scale: 1.08, boxShadow: '0 4px 24px rgba(0,212,255,0.5)' }} whileTap={{ scale: 0.95 }}
          onClick={() => setChatOpen(true)} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 1.2, type: 'spring' }}
          style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 200, width: 48, height: 48, borderRadius: '50%', background: 'rgba(0,212,255,0.15)', border: '1px solid rgba(0,212,255,0.4)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 20px rgba(0,212,255,0.2)', backdropFilter: 'blur(12px)' }}>
