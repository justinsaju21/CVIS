...
% Row 3 — Backend
\node[block, right=2.8cm of layer] (fastapi) {FastAPI\\Backend\\(Port 8000)};
...
Data flow: WebSocket \texttt{ws://localhost:8000/ws} → \texttt{event: telemetry} message → React state update → re-render gauges.
...
\midrule
8000 & FastAPI Backend & REST + WebSocket \\
